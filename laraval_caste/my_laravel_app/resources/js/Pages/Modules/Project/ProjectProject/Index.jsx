...
export default function Index({ records = [], filters = {} }) {
    ...
    const applyFilters = (newFilters) => {
        ...
        router.get("/tasks", { filters: cleaned }, { preserveState: true, replace: true });
    };
            ...
            <div className="bg-white border-b border-gray-200 px-6 py-3 shadow-2xs">
                <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    ...
                    <div className="flex items-center gap-4">
                        <h1 className="text-xl font-bold text-gray-900">ProjectProject</h1>
                        <Link
                            href="/tasks/create"
                    ...
                    <div className="flex items-center gap-3 relative">
                        ...
                        <div className="py-1">
                            <label className="block text-[11px] font-medium text-gray-600 mb-0.5">Project</label>
                            <input
                                type="text"
                                value={filtersState.project_id ?? ""}
                                onChange={(e) => updateSpecificFilter("project_id", e.target.value)}
                ...
                <div className="bg-white border border-gray-200 rounded shadow-2xs overflow-hidden">
                    <table className="min-w-full divide-y divide-gray-200 text-sm">
                        ...
                        <tbody className="divide-y divide-gray-100 bg-white">
                            ...
                            <td className="px-4 py-3 text-sm text-gray-700"><Link href={`/tasks/${record.id}`} className="font-semibold text-gray-900 hover:text-[#714B67] transition">{record.name || "-"}</Link></td>
                            ...
                            <td className="px-4 py-3 text-sm text-gray-700"><Link href={`/tasks/${record.id}`} className="font-semibold text-gray-900 hover:text-[#714B67] transition">{record.name || "-"}</Link></td>
                            <td className="px-4 py-3 text-sm text-gray-700">{record.code ?? "-"}</td>
                            <td className="px-4 py-3 text-sm text-gray-700">{record.project_id ?? "-"}</td>
                            <td className="px-4 py-3 text-sm text-gray-700">{record.is_active ? (
                                <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-emerald-100 text-emerald-800">Active</span>
                            ) : (
                                <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-600">Inactive</span>
                                        ...
                                        <td className="px-4 py-3 text-right whitespace-nowrap">
                                            <Link href={`/tasks/${record.id}`} className="text-xs font-semibold text-[#714B67] hover:underline mr-3">View</Link>
                                            <Link href={`/tasks/${record.id}/edit`} className="text-xs font-medium text-gray-600 hover:text-gray-900">Edit</Link>
                                        </td>
                                    </tr>
                                ))
                            ) : (
                                <tr>
                                    <td colSpan={9} className="px-4 py-16 text-center text-gray-500">
                                        <div className="flex flex-col items-center justify-center">
                                            ...
                                            <p className="text-base font-semibold text-gray-700">No ProjectProject records found</p>
