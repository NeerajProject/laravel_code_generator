...
export default function Index({ records = [], filters = {} }) {
    ...
    const applyFilters = (newFilters) => {
        ...
        router.get("/sale-order-lines", { filters: cleaned }, { preserveState: true, replace: true });
    };
            ...
            <div className="bg-white border-b border-gray-200 px-6 py-3 shadow-2xs">
                <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    ...
                    <div className="flex items-center gap-3 relative">
                        ...
                        <div className="py-1">
                            <label className="block text-[11px] font-medium text-gray-600 mb-0.5">Customer</label>
                            <input
                                type="text"
                                value={filtersState.customer_id ?? ""}
                                onChange={(e) => updateSpecificFilter("customer_id", e.target.value)}
                                placeholder="Filter by Customer..."
                                className="w-full text-xs rounded border-gray-300 py-1 px-2 focus:border-[#714B67] focus:ring-1 focus:ring-[#714B67]"
                            />
                        </div>
                        <div className="py-1">
                            <label className="block text-[11px] font-medium text-gray-600 mb-0.5">Order Date</label>
                            <input
                                type="text"
                                value={filtersState.order_date ?? ""}
                                onChange={(e) => updateSpecificFilter("order_date", e.target.value)}
                        ...
                        <div className="py-1">
                            <label className="block text-[11px] font-medium text-gray-600 mb-0.5">Product</label>
                            <input
                                type="text"
                                value={filtersState.product_id ?? ""}
                                onChange={(e) => updateSpecificFilter("product_id", e.target.value)}
                                placeholder="Filter by Product..."
                                className="w-full text-xs rounded border-gray-300 py-1 px-2 focus:border-[#714B67] focus:ring-1 focus:ring-[#714B67]"
                            />
                        </div>
                        <div className="py-1">
                            <label className="block text-[11px] font-medium text-gray-600 mb-0.5">Order</label>
                            <input
                                type="text"
                                value={filtersState.order_id ?? ""}
                                onChange={(e) => updateSpecificFilter("order_id", e.target.value)}
                ...
                <div className="bg-white border border-gray-200 rounded shadow-2xs overflow-hidden">
                    <table className="min-w-full divide-y divide-gray-200 text-sm">
                        <thead className="bg-gray-50/80">
                            ...
                            <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">Sale Price</th>
                            <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">Is Active</th>
                            <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">Name</th>
                            ...
                            <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">Amount Total</th>
                            ...
                            <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">Unit Price</th>
                        ...
                        <tbody className="divide-y divide-gray-100 bg-white">
                            ...
                            <td className="px-4 py-3 text-sm text-gray-700"><Link href={`/sale-order-lines/${record.id}`} className="font-semibold text-gray-900 hover:text-[#714B67] transition">{record.name || "-"}</Link></td>
                            ...
                            <td className="px-4 py-3 text-sm text-gray-700">{record.sale_price ?? "-"}</td>
                            <td className="px-4 py-3 text-sm text-gray-700">{record.is_active ? (
                                <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-emerald-100 text-emerald-800">Active</span>
                            ) : (
                                <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-600">Inactive</span>
                            ...
                            <td className="px-4 py-3 text-sm text-gray-700"><Link href={`/sale-order-lines/${record.id}`} className="font-semibold text-gray-900 hover:text-[#714B67] transition">{record.name || "-"}</Link></td>
                            <td className="px-4 py-3 text-sm text-gray-700">{record.customer_id ?? "-"}</td>
                            <td className="px-4 py-3 text-sm text-gray-700">{record.order_date ?? "-"}</td>
                            <td className="px-4 py-3 text-sm text-gray-700">{record.state ?? "-"}</td>
                            <td className="px-4 py-3 text-sm text-gray-700">{record.amount_total ?? "-"}</td>
                            <td className="px-4 py-3 text-sm text-gray-700">{record.product_id ?? "-"}</td>
                            <td className="px-4 py-3 text-sm text-gray-700">{record.quantity ?? "-"}</td>
                            <td className="px-4 py-3 text-sm text-gray-700">{record.unit_price ?? "-"}</td>
                            <td className="px-4 py-3 text-sm text-gray-700">{record.subtotal ?? "-"}</td>
                                        <td className="px-4 py-3 text-right whitespace-nowrap">
                                            <Link href={`/sale-order-lines/${record.id}`} className="text-xs font-semibold text-[#714B67] hover:underline mr-3">View</Link>
                                            <Link href={`/sale-order-lines/${record.id}/edit`} className="text-xs font-medium text-gray-600 hover:text-gray-900">Edit</Link>
                                        </td>
                                    </tr>
                                ))
                            ) : (
                                <tr>
                                    <td colSpan={19} className="px-4 py-16 text-center text-gray-500">
