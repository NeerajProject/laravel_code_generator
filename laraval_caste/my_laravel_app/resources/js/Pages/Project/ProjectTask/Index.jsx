...
export default function Index({ records, filters = {} }) {
    ...
    const submitSearch = (e) => {
        e.preventDefault();
        router.get('/project-tasks', { search }, { preserveState: true, replace: true });
    };
                    ...
                    <div className="flex items-center justify-between flex-wrap gap-3">
                        <div>
                            <nav className="text-xs text-slate-500 mb-1">
                                ...
                                <span className="text-slate-700 font-medium">Project Task</span>
                            </nav>
                            <h1 className="text-2xl font-semibold text-slate-900 tracking-tight">Project Task</h1>
                        </div>
                        <div className="flex items-center gap-2">
                            ...
                            <Link href="/project-tasks/create" className="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-md shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500/40 transition">
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="bg-slate-50 border-b border-slate-200">
                                    ...
                                    <th className="px-5 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider whitespace-nowrap">Name</th>
                                    <th className="px-5 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider whitespace-nowrap">Project Id</th>
                                    <th className="px-5 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider whitespace-nowrap">Assignee Id</th>
                                    <th className="px-5 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider whitespace-nowrap">State</th>
                                    <th className="px-5 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider whitespace-nowrap">Deadline</th>
                                    <th className="px-5 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider whitespace-nowrap">Total Hours</th>
                            ...
                            <tbody className="divide-y divide-slate-100">
                                ...
                                {records.data.map((row) => (
                                    <tr key={row.id} className="hover:bg-slate-50/70 transition-colors">
                                        ...
                                        <td className="px-5 py-3 text-slate-700 whitespace-nowrap">
                                            {row.assignee?.name || row.assignee_id || '-'}
                                        </td>
                                        <td className="px-5 py-3 text-slate-700 whitespace-nowrap">
                                            {row.state ? (
                                                <span className={
                                                    'inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ' +
                                                    (row.state === 'confirmed' ? 'bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200' :
                                                     row.state === 'cancelled' ? 'bg-rose-50 text-rose-700 ring-1 ring-rose-200' :
                                        ...
                                        <td className="px-5 py-3 text-slate-700 whitespace-nowrap">
                                            <span className="font-medium text-slate-900 tabular-nums">{Number(row.total_hours ?? 0).toFixed(2)}</span>
                                        </td>
                                        <td className="px-5 py-3 text-right whitespace-nowrap">
                                            <div className="inline-flex items-center gap-1">
                                                <Link href={`/project-tasks/${row.id}/edit`} className="inline-flex items-center justify-center w-8 h-8 text-slate-500 hover:text-blue-600 hover:bg-blue-50 rounded-md transition" title="Edit">
