...
export default function Form({ record, relations = {}, readonly = false }) {
    ...
    const { data, setData, post, put, processing, errors } = useForm({
        date: record?.date ?? '',
        task_id: record?.task_id ?? record?.task?.id ?? '',
    ...
    const submit = (e) => {
        e.preventDefault();
        if (isEdit) {
            put(`/project-timesheets/${record.id}`);
        } else {
            post('/project-timesheets');
        }
    };

    return (
        <div className="min-h-screen bg-slate-50">
            <Head title="Project Timesheet" />
                    ...
                    <div className="flex items-center justify-between flex-wrap gap-3">
                        <div>
                            <nav className="text-xs text-slate-500 mb-1">
                                <Link href="/project-timesheets" className="hover:text-slate-700">Home</Link>
                                <span className="mx-1.5">/</span>
                                <Link href="/project-timesheets" className="hover:text-slate-700">Project Timesheet</Link>
                                <span className="mx-1.5">/</span>
                                <span className="text-slate-700 font-medium">{isEdit ? 'Edit' : 'New'}</span>
                            </nav>
                            <h1 className="text-2xl font-semibold text-slate-900 tracking-tight">
                                {isEdit ? 'Edit' : 'New'} Project Timesheet
                            </h1>
                        </div>
                        <Link href="/project-timesheets" className="inline-flex items-center gap-1.5 px-3 py-2 text-sm font-medium text-slate-600 bg-white border border-slate-300 rounded-md hover:bg-slate-50 transition">
            ...
            <form onSubmit={submit} className="max-w-5xl mx-auto px-6 py-6 space-y-6">
                ...
                <div className="bg-white rounded-lg shadow-sm border border-slate-200">
                    ...
                    <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-x-5 gap-y-5">
                        ...
                        <div>
                            <label className="block text-xs font-semibold text-slate-700 mb-1.5">Date <span className="text-rose-500">*</span></label>
                            <input type="date"  required value={data.date} readOnly={readonly} onChange={(e) => setData('date', e.target.value)} className="w-full text-sm text-slate-800 bg-white border border-slate-300 rounded-md px-3 py-2 placeholder:text-slate-400 transition focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500 disabled:bg-slate-50 disabled:text-slate-500 read-only:bg-slate-50" />
                            {errors.date && <p className="mt-1 text-xs text-rose-600">{errors.date}</p>}
                        </div>

                        <div>
                            <label className="block text-xs font-semibold text-slate-700 mb-1.5">Task Id</label>
                            <select value={data.task_id} disabled={readonly} onChange={(e) => setData('task_id', e.target.value)} className="w-full text-sm text-slate-800 bg-white border border-slate-300 rounded-md px-3 py-2 placeholder:text-slate-400 transition focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500 disabled:bg-slate-50 disabled:text-slate-500 read-only:bg-slate-50">
                                <option value="">Select...</option>
                                {(relations['task_id'] || []).map((item) => (
                                    <option key={item.id} value={item.id}>{item.name}</option>
                                ))}
                            </select>
                            {errors.task_id && <p className="mt-1 text-xs text-rose-600">{errors.task_id}</p>}
                        </div>

                        <div>
                            <label className="block text-xs font-semibold text-slate-700 mb-1.5">Employee Id</label>
                            <select value={data.employee_id} disabled={readonly} onChange={(e) => setData('employee_id', e.target.value)} className="w-full text-sm text-slate-800 bg-white border border-slate-300 rounded-md px-3 py-2 placeholder:text-slate-400 transition focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500 disabled:bg-slate-50 disabled:text-slate-500 read-only:bg-slate-50">
                                <option value="">Select...</option>
                        ...
                        <div>
                            <label className="block text-xs font-semibold text-slate-700 mb-1.5">Hours <span className="text-rose-500">*</span></label>
                            <input type="number" step="0.01" required value={data.hours} readOnly={readonly} onChange={(e) => setData('hours', e.target.value)} className="w-full text-sm text-slate-800 bg-white border border-slate-300 rounded-md px-3 py-2 placeholder:text-slate-400 transition focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500 disabled:bg-slate-50 disabled:text-slate-500 read-only:bg-slate-50" />
                            {errors.hours && <p className="mt-1 text-xs text-rose-600">{errors.hours}</p>}
                        </div>

                        <div>
                            <label className="block text-xs font-semibold text-slate-700 mb-1.5">Description</label>
                            <input type="text"   value={data.description} readOnly={readonly} onChange={(e) => setData('description', e.target.value)} className="w-full text-sm text-slate-800 bg-white border border-slate-300 rounded-md px-3 py-2 placeholder:text-slate-400 transition focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500 disabled:bg-slate-50 disabled:text-slate-500 read-only:bg-slate-50" />
                ...
                {!readonly && (
                    <div className="bg-white rounded-lg shadow-sm border border-slate-200 px-6 py-4 flex items-center justify-between flex-wrap gap-3 sticky bottom-4">
                        ...
                        <div className="flex items-center gap-2">
                            <Link href="/project-timesheets" className="inline-flex items-center px-4 py-2 text-sm font-medium text-slate-700 bg-white border border-slate-300 rounded-md hover:bg-slate-50 transition">Cancel</Link>
