...
export default function Index({ records, filters = {} }) {
    ...
    const submitSearch = (e) => {
        e.preventDefault();
        router.get('/project-timesheets', { search }, { preserveState: true, replace: true });
    };

    const remove = (id) => {
        if (!confirm('Delete this record?')) return;
        router.delete(`/project-timesheets/${id}`);
    };
                    ...
                    <div className="flex items-center justify-between flex-wrap gap-3">
                        <div>
                            <nav className="text-xs text-slate-500 mb-1">
                                ...
                                <span className="text-slate-700 font-medium">Project Timesheet</span>
                            </nav>
                            <h1 className="text-2xl font-semibold text-slate-900 tracking-tight">Project Timesheet</h1>
                        </div>
                        <div className="flex items-center gap-2">
                            ...
                            <Link href="/project-timesheets/create" className="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-md shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500/40 transition">
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="bg-slate-50 border-b border-slate-200">
                                    ...
                                    <th className="px-5 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider whitespace-nowrap">Date</th>
                                    <th className="px-5 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider whitespace-nowrap">Task Id</th>
                                    <th className="px-5 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider whitespace-nowrap">Employee Id</th>
                                    <th className="px-5 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider whitespace-nowrap">Hours</th>
                                    <th className="px-5 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider whitespace-nowrap">Description</th>
                            ...
                            <tbody className="divide-y divide-slate-100">
                                ...
                                {records.data.map((row) => (
                                    <tr key={row.id} className="hover:bg-slate-50/70 transition-colors">
                                        ...
                                        <td className="px-5 py-3 text-slate-700 whitespace-nowrap">
                                            {row.task?.name || row.task_id || '-'}
                                        </td>
                                        <td className="px-5 py-3 text-slate-700 whitespace-nowrap">
                                            {row.employee?.name || row.employee_id || '-'}
                                        </td>
                                        <td className="px-5 py-3 text-slate-700 whitespace-nowrap">
                                            <span className="font-medium text-slate-900 tabular-nums">{Number(row.hours ?? 0).toFixed(2)}</span>
                                        </td>
                                        <td className="px-5 py-3 text-slate-700 whitespace-nowrap">
                                            {row.description ?? '-'}
                                        </td>
                                        <td className="px-5 py-3 text-right whitespace-nowrap">
                                            <div className="inline-flex items-center gap-1">
                                                <Link href={`/project-timesheets/${row.id}/edit`} className="inline-flex items-center justify-center w-8 h-8 text-slate-500 hover:text-blue-600 hover:bg-blue-50 rounded-md transition" title="Edit">
