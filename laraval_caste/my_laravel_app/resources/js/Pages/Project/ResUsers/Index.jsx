...
export default function Index({ records, filters = {} }) {
    ...
    const submitSearch = (e) => {
        e.preventDefault();
        router.get('/res-users', { search }, { preserveState: true, replace: true });
    };

    const remove = (id) => {
        if (!confirm('Delete this record?')) return;
        router.delete(`/res-users/${id}`);
    };

    return (
        <div className="min-h-screen bg-slate-50">
            <Head title="Res Users" />
                    ...
                    <div className="flex items-center justify-between flex-wrap gap-3">
                        <div>
                            <nav className="text-xs text-slate-500 mb-1">
                                ...
                                <span className="text-slate-700 font-medium">Res Users</span>
                            </nav>
                            <h1 className="text-2xl font-semibold text-slate-900 tracking-tight">Res Users</h1>
                        </div>
                        <div className="flex items-center gap-2">
                            ...
                            <Link href="/res-users/create" className="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-md shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500/40 transition">
                                    ...
                                    <tr key={row.id} className="hover:bg-slate-50/70 transition-colors">
                                        ...
                                        <td className="px-5 py-3 text-right whitespace-nowrap">
                                            <div className="inline-flex items-center gap-1">
                                                <Link href={`/res-users/${row.id}/edit`} className="inline-flex items-center justify-center w-8 h-8 text-slate-500 hover:text-blue-600 hover:bg-blue-50 rounded-md transition" title="Edit">
